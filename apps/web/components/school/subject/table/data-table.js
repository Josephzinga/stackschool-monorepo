...
exports.SubjectDataTable = SubjectDataTable;
...
function SubjectDataTable({ columns, data, meta, isLoading, }) {
    const { pagination, setPagination } = (0, table_provider_1.useSubjectTable)();
    ...
    return (<div className="w-full h-full font-poppins z-10 flex flex-col gap-4">
      <div className="rounded-md mb-2 border relative min-h-75 overflow-x-auto">
        {isLoading && (<div className="absolute inset-0 bg-white/50 dark:bg-slate-800/70 z-50 flex flex-col backdrop-blur-sm">
            {Array.from({ length: 12 }).map((_, i) => (<div key={i} className="flex justify-between items-center flex-row animate-pulse px-4 w-full h-14 even:bg-slate-50 dark:even:bg-slate-950">
