...
export const verifyCodeController = async (
  ...
  try {
    ...
    try {
      ...
      return res.status(400).json({ ok: false, message: 'Token expiré ou invalide' });
    }
    ...
    if (!verificationCode) {
      return res.status(404).json({ ok: false, message: 'Code invalide ou expiré' });
    }
    ...
    return next(createServiceError('Erreur lors de la vérification du code', 500, err));
  }
};
