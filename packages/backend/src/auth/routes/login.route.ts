...
router.post("/login", (req, res, next) => {
  passport.authenticate(
    "local",
    async (err: any, user: any | false, info: any) => {
      ...
      try {
        if (err) {
          ...
          return res.redirect(
            `${process.env.FRONTEND_ORIGIN}/auth/login?error=server`
          );
        }
        ...
        if (info?.isSocialOnly) {
          ...
          }/auth/finish?from=social&providers=${encodeURIComponent(providers)}`;
