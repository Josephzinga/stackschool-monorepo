...
interface LectureCardProps {
  classe: string;
  time: string; // Heure prévue
  realStartTime?: string; // Heure réelle via Socket.io
  title: string;
  subject: string;
  chapter: string;
  teacher?: {
...
export function LectureCard({
  ...
  return (
    <div
      className={`bg-background space-y-2 rounded-lg px-4 py-2 border-l-4 transition-all ${
        isLive
    ...
    >
      ...
      <div className="flex items-center gap-1 text-sm bg-muted/30 rounded">
        <LucideCircleCheck
          className={isLive ? 'text-green-500' : 'text-muted-foreground'}
          size={18}
        />
