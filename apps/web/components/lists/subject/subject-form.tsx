...
import { createSubjectForm, CreateSubjectForm } from '@stackschool/contracts';
...
export default function SubjectForm({
  ...
  const {
    ...
    resolver: zodResolver(createSubjectForm),
    defaultValues: {
      ...(initialValues && {
        ...initialValues,
      }),
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-3">
                      ...
                      <SelectItem key={teacher?.id} value={teacher?.id!}>
                        <div className="flex flex-col">
                          <span className="text-sm font-inter font-medium">
                            {teacher?.schoolProfile?.lastName}
                          </span>
                          <span className="text-[10px] opacity-80 font-poppins">
                            {teacher?.schoolProfile?.firstName}
                          </span>
                        </div>
                      </SelectItem>
                    ))}
              ...
              return (
                ...
                >
                  ...
                  <div className="w-20 space-y-1">
                    ...
                    <Controller
                      control={control}
                      name={`classSubjects.${index}.coefficient`}
                      render={({ field: { onChange, value } }) => (
                        <Input
                          type="number"
                          min="1"
                          ...
                          aria-invalid={
                            !!errors.classSubjects?.[index]?.coefficient
                          }
                          value={value}
                          onChange={(event) =>
                    ...
                    />
                    <FieldError>
                      {errors.classSubjects?.[index]?.coefficient?.message}
                    </FieldError>
                  </div>
                  ...
                  <div className="w-20 space-y-1">
                    ...
                    <Controller
                      control={control}
                      name={`classSubjects.${index}.weeklyHours`}
                      render={({ field: { onChange, value } }) => (
                        <Input
                          type="number"
                          min="1"
                          className="h-8! text-sm"
                          aria-invalid={
                            !!errors.classSubjects?.[index]?.weeklyHours
                          }
                          value={value}
                          onChange={(event) =>
                            onChange(Number(event.target.value))
                    ...
                    <FieldError className="text-[8px]!">
                      {errors.classSubjects?.[index]?.weeklyHours?.message}
                    </FieldError>
                  </div>
        ...
        >
          ...
          <DialogContent className="max-w-90!">
            <DialogHeader>
              <DialogTitle>Selectionné des classes</DialogTitle>
              <DialogDescription>
