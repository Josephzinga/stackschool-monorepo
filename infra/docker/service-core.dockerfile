...
RUN npx nx build service-core

FROM node:lts-slim AS runner
ENV NODE_ENV=production
WORKDIR /repo
COPY --from=builder /repo/node_modules ./node_modules
COPY --from=builder /repo/apps/service-core ./apps/service-core
COPY --from=builder /repo/packages/messaging ./packages/messaging
COPY --from=builder /repo/packages/contracts ./packages/contracts
COPY --from=builder /repo/service-core/package.json ./service-core/package.json
CMD ["node", "apps/service-core/dist/main.js"]
